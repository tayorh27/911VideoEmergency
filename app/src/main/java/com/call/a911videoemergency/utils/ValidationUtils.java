package com.call.a911videoemergency.utils;
...
public class ValidationUtils {
    ...
    private static boolean isEnteredTextValid(Context context, EditText editText, int resFieldName, int maxLength, boolean checkName) {
        ...
        Pattern p;
        if (checkName) {
            p = Pattern.compile("^[a-zA-Z][a-zA-Z 0-9]{1," + (maxLength - 1) + "}+$");
        } else {
            p = Pattern.compile("^[a-zA-Z][a-zA-Z0-9]{1," + (maxLength - 1) + "}+$");
        }
    ...
    public static boolean isRoomNameValid(Context context, EditText editText) {
        return isEnteredTextValid(context, editText, R.string.field_name_chat_room_name, 90, false);
    }
}
